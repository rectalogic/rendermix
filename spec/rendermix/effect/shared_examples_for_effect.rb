...
shared_examples 'a transition effect' do
  ...
  it 'should apply to overlapping Sequences in Parallel' do
    seq1_dur = 5*4
    seq1 = @app.mixer.new_sequence
    seq1.append(@app.mixer.new_image(FIXTURE_IMAGE, seq1_dur))

    seq2_start = 5*2
    seq2 = @app.mixer.new_sequence
    seq2.append(@app.mixer.new_blank(seq2_start))
    seq2.append(@app.mixer.new_media(FIXTURE_MEDIA, duration: 5*8))
  ...
  it 'should apply to Sequence overlapping Image in Parallel' do
    ...
    seq.append(@app.mixer.new_blank(seq_start))
    seq.append(@app.mixer.new_media(FIXTURE_MEDIA, duration: 5*4))
